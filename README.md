...
## Results
After the implementation was complete, we ran a rudimentary test. We strapped our boards to our wrists using rubber bands and connected the board to the laptop via USB. Then, we mimed the different actions for little over 10 minutes (or 200 inference cycles) while keeping tally of the misclassifications. Overall, we found that the real world accuracy was around 53% whereas the original model was reported to have 59% accuracy.
